...
import { Title, View, Close, Cell, Check, Loading } from '../components';
...
const State = state => {
  return {
    check: state.check,
    loading: state.loading.global || state.docs.length === 0,
    docs: state.docs,
  };
};
...
class Docs extends Component {
  ...
  DocsView = () => (
    <>
      <Panel key="panel">{this.props.docs.map(this.mapData)}</Panel>
      <LibraryView key="lib">
        {this.state.search ? (
          <this.SearchResult />
        ) : (
          this.props.docs[this.state.activeRoot].group[this.state.activeGroup].group.map(
            this.mapLib
          )
        )}
      </LibraryView>
  ...
  CheckView = () => (this.props.loading ? <Loading /> : <this.DocsView />);
