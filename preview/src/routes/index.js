...
class App extends Component {
  ...
  List = page => {
    return (
      <Cell key={page.name}
        ...
        <Cover path={page.path}></Cover>
        <Content>
          <Title><Tag mode={page.mode}>{page.mode}</Tag>{page.name}</Title>
          <Date><Icon type="clock-circle"/> {page.date[0]}{page.date[1]}-{page.date[2]}{page.date[3]}</Date>
        </Content>
      </Cell>
    );
  };
  ...
  Page = ({ data }) => {
    ...
    return (
      <Privew>
        <List>
          <Header>
            <a className="logo" href="https://www.yuque.com/canisminor/anto/readme" target="_blank" />
          </Header>
        ...
        <Showcase>
          <Header>
            <Group>
              <Zoom>
                <ZoomIcon onClick={this.handleMinus}><Icon type="minus"/></ZoomIcon>
                <ZoomNum onClick={this.handleReset}>{Math.floor(this.state.zoom * 100)}%</ZoomNum>
                <ZoomIcon onClick={this.handlePlus}><Icon type="plus"/></ZoomIcon>
              </Zoom>
              <PageTitle>{this.state.name} ({this.state.mode})</PageTitle>
            </Group>
            <Author>
  ...
  render() {
    return (
      <View>
        {this.props.loading ? "Loading..." : <this.Page data={this.props.preview}/>}
      </View>
    );
  }
